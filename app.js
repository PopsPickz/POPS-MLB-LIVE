...
async function buildBatterTargets(games) {
  ...
  if (batterTargetCache[todayKey]) return batterTargetCache[todayKey];
  ...
  batterTargetCache[todayKey] = targets;
  return targets;
}

function renderHRPicks(targets) {
  targets = [...targets]
...
function renderHitPicks(targets) {
  targets = [...targets]
    .filter(p => Number(p.hitStreak || 0) >= 2 || Number(p.bvpHR || 0) > 0)
    .sort((a, b) => {
      const streakDiff = Number(b.hitStreak || 0) - Number(a.hitStreak || 0);
...
async function loadMoneyline(games) {
  ...
  for (const game of games) {
    try {
      ...
      const confidence = pick === "No Clear Edge"
        ? 50
        : Math.max(55, Math.min(100, Math.max(awayScore, homeScore)));
...
async function loadApp() {
  ...
  try {
    ...
    const targets = await buildBatterTargets(games);

    renderHRPicks(targets);
    renderHitPicks(targets);
